...
local function moat_BossPlayerDeath(ply)
    ...
    if (not IS_BOSS) then
		cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/apache/apache_loss_smith.mp3", 2)
		if (IsValid(MOAT_BOSS_CUR_PLY)) then
            MOAT_BOSS_CUR_PLY:m_DropInventoryItem(math.random(5,6))
        end
        return
    else
		cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/apache/apache_won_smith.mp3", 2)
    end
...
local function moat_BeginRoundBossHooks()
    ...
    hook.Add("ttt.BeginRound", "moat_BossBeginRound", function()
        ...
        timer.Simple(1, function()
            MuteForRestart(true)
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/apache/apache1smith.mp3", 2)
        end)
