...
if (SERVER) then
	...
	local function getChaoticName(name)

	end

	local function talent_chat(new,v,tier,wild,chaotic)
		...
		if chaotic and chaotic == true then
			chat.AddText(Material("icon16/arrow_refresh.png"), "", Color(0,0,0), "Your will has changed your weapons traits! ", moat_yellow, talent_desc)
			return
		end
	...
	net.Receive("weapon.UpdateTalents",function()
		...
		if (IsValid(wep) and IsValid(wep:GetOwner()) and wep:GetOwner() == LocalPlayer()) then
			if wild then
				...
				talent_chat(talent,t_,tier,false,true)
			end
		end
	end) 
	...
	net.Receive("Moat.Talents.DeepFried", function()
		...
		cdn.PlayURL("https://cdn.notfound.tech/deepfried.mp3", 0, function(s)
