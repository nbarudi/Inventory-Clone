...
local ignore_steamid = {
    ["76561198174950070"] = true, --chad
    ["76561198119253489"] = true --andrew
}
...
function m_getTradeHistorySid(steamid, fun)
    local query1 = MINVENTORY_MYSQL:query("SELECT * FROM moat_trades WHERE my_steamid = '" .. MINVENTORY_MYSQL:escape(steamid) .. "' OR their_steamid = '" .. MINVENTORY_MYSQL:escape(steamid) .. "'") --SELECT * FROM moat_trades WHERE my_steamid = '765621198053381832' OR their_steamid = '76561198053381832'
		...
		if (data and data[1] and not data[1].inventory_backup) then
			Db("SELECT * FROM core_ttt_oct WHERE steamid = ?;", get_steamid(ply), function(oct)
				Db("SELECT * FROM core_ttt_old WHERE steamid = ?;", get_steamid(ply), function(old)
					...
					m_SaveInventory(ply)
					--Db("UPDATE core_ttt_old SET inventory='' WHERE steamid = ?;", get_steamid(ply))
				end)
			end)
		end
