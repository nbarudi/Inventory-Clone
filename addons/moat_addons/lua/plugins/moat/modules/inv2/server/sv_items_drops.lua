...
function meta:m_DropInventoryItem(cmd_item, cmd_class, drop_cosmetics, delay_le_saving, hide_chat, dev_talent_tbl, dev_stat_tbl)
    local dropped_item = {}
	...
	if (dropped_item and dropped_item.u and item_to_drop and item_to_drop.Rarity and not dont_show_chat) then
		if (item_to_drop.Rarity == 6) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_04.wav", .5)
			util.GlobalScreenShake(5, 5, 10, 3000)
		elseif (item_to_drop.Rarity == 7) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_07.wav", .5)
			util.GlobalScreenShake(5, 5, 10, 3000)
		elseif (item_to_drop.Rarity == 8) then
			self:PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_03.wav", .5)
			self:ScreenShake(5, 5, 10, 3000)
		elseif (item_to_drop.Rarity == 9) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Arcana_drop.mp3", .5)
			util.GlobalScreenShake(25, 25, 15, 3000)
        elseif (item_to_drop.Rarity == 10 or item_to_drop.Rarity == 11) then
            cdn.PlayURL("http://tttweb.bungo.ca/public/ttt/drops/chaotic.wav", .5)
            util.GlobalScreenShake(50, 50, 30, 3000)
		elseif (tonumber(dropped_item.u) == 912 or item_to_drop.Name:match '^Titan T') then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Titan_Drop.mp3", .5)
		end
	end
...
function m_GetRandomRarity(min, max)
    ...
    for i = min, max do
        if(MOAT_RARITIES[i+1] == nil) then
            chosen_rarity = MOAT_RARITIES[i].ID
            break
        end
        local chance_to_move = math.random(MOAT_RARITIES[i + 1].Chance)

        if (chance_to_move ~= MOAT_RARITIES[i + 1].Chance) then
            chosen_rarity = MOAT_RARITIES[i].ID
            break
        end
	end
...
concommand.Add("moat_drop_item", function(ply, cmd, args)
    ...
    if (args[3]) then
        if (args[3] == "all") then
                local msg = string(":gift: " .. style.Bold(ply:Nick()) .. style.Dot(style.Code(ply:SteamID())) .. style.Dot(ply:SteamURL()), style.NewLine(":tada: Just dropped a gift for the server ") .. "on " .. string.Extra(GetServerName(), Servers.SteamURL .. GetServerIP()))
                discord.Send("Event", msg)
                for k, v in pairs(player.GetAll()) do
                    v:m_DropInventoryItem(args[1], args[2])
