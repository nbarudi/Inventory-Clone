...
hook.Add("EntityTakeDamage", "moat_ApplyDamageMods", function(ent, dmginfo)
    ...
    if(ent.UnlimVoid) then
        print("Damged with Unlim Void Active! " .. ent.UnlimVoid.Reduction)
        local ogDamage = dmginfo:GetDamage()
        local newDamage = ogDamage * ent.UnlimVoid.Reduction
        dmginfo:SetDamage(newDamage)
        ent.UnlimVoid.Damage = ent.UnlimVoid.Damage + ogDamage-newDamage
    end
end)
        ...
        for i = 1, level_upgrades do
            if ((cur_lvl + i) % 2 == 0) then
                ...
                if (player.GetCount() > 4) then
                    attacker:m_DropInventoryItem(crate, "hide_chat_obtained")
                end
                we_saved = true
            end
        end
