MOAT_VERSION = "1.7.1"
MOAT_BG_URL = "https://tttweb.bungo.ca/public/ttt/misc/bg.png"
...
local halloween_bg = "https://tera.gg/ttt/misc/halloween.png"
...
local spring_bg_url = "https://tttweb.bungo.ca/public/ttt/misc/spring_bg.png"
local springs = {
    [1] = {"butterfly1.png", 0, 0, 0, url = "https://tttweb.bungo.ca/public/ttt/misc/butterfly2.png"},
    [2] = {"butterfly2.png", 0, 0, 0, url = "https://tttweb.bungo.ca/public/ttt/misc/butterfly2.png"},
    [3] = {"butterfly3.png", 0, 0, 0, url = "https://tttweb.bungo.ca/public/ttt/misc/butterfly3.png"},
    [4] = {"butterfly4.png", 0, 0, 0, url = "https://tttweb.bungo.ca/public/ttt/misc/butterfly4.png"}
}
...
function m_DrawChaoticAngelicText(text, font, x, y, rarity)
    local tx = text
    local col1 = Color(0,0,0,255)
    local col2 = Color(255,255,255,255)

    if(rarity == 11) then
        if(tx == "Inner Chaos") then
            tx = "Inner Peace"
        elseif (tx == "Chaotic Will") then
            tx = "Star Citizen"
        elseif (tx == "Black Market") then
            tx = "Heavens Utopia"
        elseif (tx == "Inner Flame") then
            tx = "Holy Light"
        elseif (tx == "Draining") then
            tx = "Holy Grace"
        elseif (tx == "Black Ice") then
            tx = "Polar Ice"
        end
        col1 = Color(252,203,45)
    end
    //ToDo: Add all the talent names and others
    m_DrawEnchantedText(tx, font, x, y, col1, col2, nil, nil, true)
...
function m_DrawItemStats(font, x, y, itemtbl, pnl)
    ...
    if (itemtbl.t) then
        ...
        for k, v in ipairs(itemtbl.t) do
            ...
            elseif (tfx == "chaotic") then
                m_DrawChaoticAngelicText(talent_name, font, draw_name_x, draw_name_y, itemtbl.item.Rarity)
            ...
            if (itemtbl.s.l < talent_level) then
				...
				cdn.DrawImage("https://tttweb.bungo.ca/public/ttt/misc/icon-lock.png", 4 + draw_name_x + talent_namew, y + stats_y_add + talents_y_add + 2 + 3, 10, 10, Color(0, 0, 0, 255))
				cdn.DrawImage("https://tttweb.bungo.ca/public/ttt/misc/icon-lock.png", 3 + draw_name_x + talent_namew, y + stats_y_add + talents_y_add + 2 + 2, 10, 10, Color(100, 100, 100, 255))

				talent_namew = talent_namew + 8 + 4
            else
				local lx, ty = 4 + draw_name_x + talent_namew, y + stats_y_add + talents_y_add + 2 + 2
				cdn.DrawImage("https://tttweb.bungo.ca/public/ttt/misc/icon_star.png", lx, ty, 12, 12, Color(0, 0, 0, 255))
				cdn.DrawImage("https://tttweb.bungo.ca/public/ttt/misc/icon_star.png", lx - 1, ty - 1, 12, 12, Color(255, 255, 255, 255))
...
rarity_names = {
    ...
    [10] = { "Chaotic", Color(182,27,0,255), { min = 32500, max = 64600} },
    [11] = { "Angelic", Color(250,220,123), { min = 32500, max = 64600} }
}
...
rarity_accents = readonly_rarities {
    ...
    [9] = Color(0, 0, 0, 255),
    [10] = Color(125, 0, 0, 255),
    [11] = Color(255, 250, 176)
}
...
rarity_shadow = readonly_rarities {
    ...
    [10] = {Color(182,27,0,255), Color(255,72,0,157)},
    [11] = {Color(0,0,0), Color(0,0,0,157)}
}
...
MOAT_CRATE_DEBOUNCE = MOAT_CRATE_DEBOUNCE or false 
...
function m_OpenInventory(ply2, utrade)
	...
	MOAT_INV_BG = ux.Create("DFrame", function(s)
		...
		--cdn.DrawImage("https://notdrawn.gg/f/413567786280d852aaf0abec6e188865.png", 0, 0, 1024, 1024, Color(255, 255, 255, 250))
    ...
    for k, v in ipairs(MOAT_INV_CATS) do
        ...
        MOAT_CAT_BTN.Paint = function(s, w, h)
			 if (s.CatLabel == "Store") then
			 	cdn.DrawImage("https://em-content.zobj.net/source/apple/354/dollar-banknote_1f4b5.png", (w/2)-8, (h/2)-8, 16, 16, Color(255, 255, 255, 100 + s.hover_coloral))
	...
	function m_InventoryPanel(anim_time)
		...
		function m_CreateInvSlot(num)
			...
			m_DPanelBTN.OnMousePressed = function(s, key)
                ...
                if( key == MOUSE_MIDDLE and m_Inventory[num].c) then
                    if(m_Inventory[num].item.Kind == "Crate") then
                        if(!MOAT_CRATE_DEBOUNCE) then
                            m_OpenCrate( m_Inventory[num], true )
                            MOAT_CRATE_DEBOUNCE = true 
                            timer.Create("moat_fast_decon_debounce", 0.25, 1, function() MOAT_CRATE_DEBOUNCE = false end)
                        end
                    end
                end
				...
				if (key == MOUSE_LEFT and input.IsKeyDown(KEY_LCONTROL) and m_Inventory[num].c) then
                    ...
                    local rar = GetConVar("moat_mass_deconstruct_rarity"):GetString()

					if (m_Inventory[num].item and m_Inventory[num].item.Rarity and m_Inventory[num].item.Rarity > ITEM_RARITY_TO_NAME[rar]) then return end
					
					moat_RemoveEditPositionPanel()
                                                                                                //Max of 50 crates
                                                                                                //and table.HasValue(MOAT_VIP, LocalPlayer():GetUserGroup())
                    if(m_Inventory[num].item.Kind == "Crate" and MOAT_ITEMS_DECON_MARKED <= 0 and MOAT_CRATE_OPEN_MARKED < 1) then
			...
			m_DPanelBTN.OnCursorEntered = function(s)
                ...
                if (input.IsMouseDown(MOUSE_MIDDLE) and m_Inventory[num].c and not INV_SELECT_MODE) then
					s.OnMousePressed(s, MOUSE_MIDDLE)
				end
...
function m_CreateItemMenu(num, ldt)
    ...
    M_INV_MENU2:AddOption("Share Screenshot",function()
        ...
        hook.Add("PostRender", "Gay render capture rules", function()
			...
			HTTP({
                ...
                method = "post",
                headers = {
                    ["Authorization"] = "Client-ID 1a786503d8b317d"
                },
                success = function(_,c,_,_)
                    local album = util.JSONToTable(c)
                    if not album.success then
                        Derma_Message("Your upload was not successful (-2)! Please show this to chad: " .. a, "Imgur failed", "Close")
                    ...
                    HTTP({
                        ...
                        success = function(_,b,_,_)
                            ...
                            if b.success then
                                ...
                                Derma_Message("Your upload was not successful! Please show this to chad:\n" .. ob, "Imgur failed", "Close")
                            end
                            MOAT_UPLOADING = false
                        end,
                        failed = function(a) 
                ...
                failed = function(a) 
                    Derma_Message("Your upload was not successful (-1)! Please show this to chad: " .. a, "Imgur failed", "Close")
                    MOAT_UPLOADING = false
                end,
                parameters = {
	...
	if (string.find(string.lower(LocalPlayer():Nick()), "bungottt.gg")) then
...
net.Receive("MOAT_ITEM_OBTAINED", function(len)
	...
	if (last_played <= CurTime() - .5) then
		if (tbl.item and tbl.item.Rarity == 2) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_01.wav", .2)
		elseif (tbl.item and tbl.item.Rarity == 3) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_02.wav", .2)
		elseif (tbl.item and tbl.item.Rarity == 4) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_06.wav", .2)
		elseif (tbl.item and tbl.item.Rarity == 5) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/Stinger_loot_05.wav", .2)
		elseif (tbl.item and tbl.item.Name and string.EndsWith(tbl.item.Name, "Mutator")) then
			cdn.PlayURL("https://tttweb.bungo.ca/public/ttt/misc/tttsounds/legendarydrop.mp3", .2)
		end
...
net.Receive("MOAT_DECON_NOTIFY", function()
	...
	if (decon_last_played <= CurTime() - .5) then
		cdn.PlayURL "https://tttweb.bungo.ca/public/ttt/misc/tttsounds//decon_notify.mp3"
