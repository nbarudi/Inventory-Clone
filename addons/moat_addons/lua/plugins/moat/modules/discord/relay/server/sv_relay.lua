...
rarity_names = {
    ...
    [11] = { "Angelic", Color(252,193,0), { min = 32500, max = 64600} }
}
...
function SVDiscordRelay.SendToDiscord(ply, s_image, s_text, teamchat)
    ...
    local nick = "Bungo TTT" --DiscordRelay.ServerPrefix .. " " .. (ply:Nick() or ply:SteamID())
...
function SVDiscordRelay.PrintToChat(code, body, headers)
    ...
    if body.message == "You are being rate limited." then
        ...
        SVDiscordRelay.SendToDiscord("BungoGaming Bot", false, "The bot is being rate limited! Players on the server will not see your messages for another " .. body.retry_after .. " seconds.")
...
function SVDiscordRelay.AnnounceConnect(ply)
    ...
    if DiscordRelay.AnnounceConnect then
        SVDiscordRelay.SendToDiscordRaw("BungoGaming Bot", false, msg)
    end
end
