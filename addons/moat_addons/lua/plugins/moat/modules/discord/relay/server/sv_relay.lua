...
function SVDiscordRelay.SendToDiscord(ply, s_image, s_text, teamchat)
    ...
    local nick = "Gritsky TTT" --DiscordRelay.ServerPrefix .. " " .. (ply:Nick() or ply:SteamID())
...
function SVDiscordRelay.PrintToChat(code, body, headers)
    ...
    if body.message == "You are being rate limited." then
        ...
        SVDiscordRelay.SendToDiscord("GritskyGaming Bot", false, "The bot is being rate limited! Players on the server will not see your messages for another " .. body.retry_after .. " seconds.")
...
function SVDiscordRelay.AnnounceConnect(ply)
    ...
    if DiscordRelay.AnnounceConnect then
        SVDiscordRelay.SendToDiscordRaw("GritskyGaming Bot", false, msg)
    end
end
