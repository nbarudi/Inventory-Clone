...
local function FindMessageBoxWidth(text)
	...
	return tw + 14
        ...
        function CommunityCheckStatus( ply )
                if not IsValid(ply) then return end
                local id = util.SteamIDTo64( ply:SteamID() )
                http.Fetch("http://steamcommunity.com/groups/bungotttgaming/memberslistxml/?xml=1", 
                        function( body, _, _, _ )
...
local function CheckGroup(id)
    chat.AddText(Color(255, 0, 0), "Currently bugged, sorry we're working on it!")
...
local function GiveRewards()
    ...
    btn.DoClick = function(s)
		...
		gui.OpenURL "https://steamcommunity.com/groups/bungotttgaming"
    end
