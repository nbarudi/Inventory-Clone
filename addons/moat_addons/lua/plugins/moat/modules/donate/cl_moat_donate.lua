...
function MOAT_DONATE:DrawRewardsInfo(pnl, pkg, clr)
    ...
    function steam:Paint(w, h)
        ...
        surface.DrawTexturedRect(0, 0, w, 45)
        surface.DrawOutlinedRect(0, 0, w, 45)
        cdn.DrawImageRotated("https://dredd.me/img/bungotttlogo512.png", 10, 4, 35, 35, nil, math.sin(CurTime() + 10) * 15, true)
    ...
    if not hastag then
        lb:SetText("NOTE: You can only get daily rewards by adding bungottt.gg to your name! Capitalization doesn't matter")
    else
        lb:SetText("You have bungottt.gg in your name and are currently receiving daily rewards!")
    end
...
function MOAT_DONATE:OpenWindow()
    ...
    self.bg.Paint = function(s, w, h)
		...
		local w2, h2 = draw.SimpleText("Bungo", "moat_GambleTitle", 4, 0, Color(0, 198, 255))
    ...
    g.DoClick = function(s)
        --gui.OpenURL("https://store.bungottt.gg")
    end
