...
function moat_DropPumpkin(ply, amt)
    ...
    for i = 1, amt do
        ...
        ply:m_DropInventoryItem("Pumpkin Crate", "hide_chat_obtained")
    end
end
...
function moat_DropHoliday(ply, amt)
    for i = 1, amt do
        if (not IsValid(ply)) then return end
        timer.Simple(0.01 * i, function() ply:m_DropInventoryItem("Holiday Crate", "hide_chat_obtained") end)
    end
end

function meta:Drop100()
	timer.Simple(1, function() self:m_DropInventoryItem("Name Mutator", "hide_chat_obtained") end)
    timer.Simple(2, function() self:m_DropInventoryItem("Name Mutator", "hide_chat_obtained") end)
    timer.Simple(3, function() self:m_DropInventoryItem("Cosmic Stat Mutator", "hide_chat_obtained") end)
    timer.Simple(4, function() self:m_DropInventoryItem("Ascended Talent Mutator", "hide_chat_obtained") end)
end
...
MOAT_DONATE.Packages = {
    ...
    [4] = {
        ...
        function(pl)
            ...
            pl:m_DropInventoryItem("Independence Crate", "hide_chat_obtained")
            for i = 1, 15 do
                local crate = crates[math.random(1, #crates)].Name
                pl:m_DropInventoryItem(crate, "hide_chat_obtained")
            end
    ...
    [9] = {
        ...
        function(ply)
            ...
            discord.Send("Gritsky TTT Announcement", markdown.WrapBold(string(":satellite_orbital::satellite: ", markdown.Bold"Global TTT Announcement", " :satellite::satellite_orbital:", markdown.LineStart(msg))))
...
local function namerewards()
    ...
    local function nametag(ply)
        return ply:Nick():lower():match("gritsky%.gg")
    end
    ...
    net.Receive("NameRewards.Collect", function(l, ply)
        ...
        ply.pendingname = nil
        q:start()
        ply:SendLua("chat.AddText('You have collected your points! Thanks for supporting Gritsky Gaming <3')")
    end)
end
