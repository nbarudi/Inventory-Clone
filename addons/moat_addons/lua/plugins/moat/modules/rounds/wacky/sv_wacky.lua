...
function moat_random.start_round(name)
    ...
    name = string.sub(name, 1, name:len())
    print(name)
...
moat_random.register("Low Gravity", "Moon gravity on Earth?", {
    ["NOW"] = function()
        game.ConsoleCommand("sv_gravity 70\n")
        net.Start("RandomRound")
    		net.WriteString("Low Gravity")
    		net.WriteString("Moon Gravity on Earth!")
    	net.Broadcast()
    end
}, function()
...
moat_random.register("Sudden Death", "Base health is 1 HP for this round!", {
    ["NOW"] = function()
		for k, v in pairs(player.GetAll()) do
			if ((v.MaxHealth and v.MaxHealth == 1) or v:GetMaxHealth() == 1 or v:Health() == 1) then
				continue
			end
		...
		net.Start("RandomRound")
    		...
    		net.WriteString("Base health is 1 HP for this round!")
    	net.Broadcast()
    end,
    ["TTTBeginRound"] = function()
        for k, v in pairs(player.GetAll()) do
