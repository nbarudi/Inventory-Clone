...
MOTD = MOTD or {}
MOTD.SpawnSettings = GetConVar "bungottt_disable_motd"
MOTD.OpenTime = 0
MOTD.CurTab = 1
MOTD.Tabs = {
	[1] = {"Rules", "https://bungo.ca"},
	[2] = {"Donate", "https://store.bungottt.gg"},
	[3] = {"Steam Group", "https://steamcommunity.com/groups/bungotttgaming"},
	[4] = {"Discord", "https://discord.gg/vZa4yQNbxk"},
	[5] = {"Close", ""},
}

function MOTD.Closable()
	return math.ceil(cookie.GetNumber("bungottt_motd_wait", 0)) <= 0
end
...
function MOTD.Open(secs, invalid, changes, new_changes)
	...
	if (secs <= -1) then
		return cookie.Set("bungottt_motd_wait", 0)
	end
	...
	if (secs and secs > 0) then
		cookie.Set("bungottt_motd_wait", cookie.GetNumber("bungottt_motd_wait", 0) + secs)
	end
    ...
    c.Paint = function(s, w, h)
        cdn.DrawImage("https://tttweb.bungo.ca/public/img/logo.png", 35, 35, 170/2, 118/2)
    end
    MOTD.w = vgui.Create("DHTML", p)
    MOTD.w:DockMargin(0, 10, 10, 10)
    MOTD.w:Dock(FILL)
    MOTD.w:OpenURL("https://bungo.ca")
    ...
    for i = 1, #MOTD.Tabs do
        ...
        btn.Paint = function(s, w, h)
            ...
            if (i == #MOTD.Tabs) then
                ...
                draw_SimpleTextOutlined(MOTD.Closable() and s.Label or util.Upper(util.FormatTimeSingle(math.ceil(cookie.GetNumber("bungottt_motd_wait", 0)), false)).. " Left", "Trebuchet24", w/2, h/2, Color(255, 50 + (205 * s.LerpNum), 50 + (205 * s.LerpNum)), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 35))

				if (s.Time and s.Time <= CurTime() and not MOTD.Closable()) then
					s.Time = CurTime() + 1

					cookie.Set("bungottt_motd_wait", math.max(cookie.GetNumber("bungottt_motd_wait", 0) - 1, 0))
				end
			else
        ...
        btn.DoClick = function(s)
            ...
            if (i == #MOTD.Tabs and MOTD.Closable()) then
				cookie.Set("bungottt_motd_wait", 0)
