...
MOTD = MOTD or {}
MOTD.SpawnSettings = GetConVar "gritsky_disable_motd"
MOTD.OpenTime = 0
MOTD.CurTab = 1
MOTD.Tabs = {
...
function MOTD.Closable()
	return math.ceil(cookie.GetNumber("gritsky_motd_wait", 0)) <= 0
end
...
function MOTD.Open(secs, invalid, changes, new_changes)
	...
	if (secs <= -1) then
		return cookie.Set("gritsky_motd_wait", 0)
	end
	...
	if (secs and secs > 0) then
		cookie.Set("gritsky_motd_wait", cookie.GetNumber("gritsky_motd_wait", 0) + secs)
	end
    ...
    for i = 1, #MOTD.Tabs do
        ...
        btn.Paint = function(s, w, h)
            ...
            if (i == #MOTD.Tabs) then
                ...
                draw_SimpleTextOutlined(MOTD.Closable() and s.Label or util.Upper(util.FormatTimeSingle(math.ceil(cookie.GetNumber("gritsky_motd_wait", 0)), false)).. " Left", "Trebuchet24", w/2, h/2, Color(255, 50 + (205 * s.LerpNum), 50 + (205 * s.LerpNum)), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 35))

				if (s.Time and s.Time <= CurTime() and not MOTD.Closable()) then
					s.Time = CurTime() + 1

					cookie.Set("gritsky_motd_wait", math.max(cookie.GetNumber("gritsky_motd_wait", 0) - 1, 0))
				end
			else
        ...
        btn.DoClick = function(s)
            ...
            if (i == #MOTD.Tabs and MOTD.Closable()) then
				cookie.Set("gritsky_motd_wait", 0)
