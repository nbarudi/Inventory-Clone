...
MGA.CommandList = {
	...
	{70, "Senior Administrator", Color(102, 0, 102, 255),  {
		...
		{"SetKarma", "Changes a players karma.", true, {
			{"Entry", "SteamID", "STEAM_0:0:"},
			{"Entry", "Karma", "2000"}
		}},
	...
	{80, "Head Administrator", Color(51, 0, 51, 255), {
		...
		{"SetWepLevel", "Set Weapon Level", true, {
			{"Entry", "SteamID", "STEAM_0:0:"},
			{"Drop", "Weapon Slot", {}, "primary"},
			{"Entry", "Weapon Level", "100"}
		}},
		{"SpawnTiered", "Spawn in a Tiered Item", true, {
			{"Entry", "SteamID", "STEAM_0:0:"},
			{"Drop", "Tiered Item", {}, "Junky"},
			{"Drop", "Weapon Type", {}, "random"},
			{"Drop", "Custom Stats", {"True", "False"}, "False"},
			{"Entry", "Damage Stat (%)", "0"},
			{"Entry", "RPM Stat (%)", "0"},
			{"Entry", "Ammo Stat (%)", "0"},
			{"Entry", "Accuracy Stat (%)", "0"},
			{"Entry", "Kick Stat (%)", "0"},
			{"Entry", "Range Stat (%)", "0"},
			{"Entry", "Weight Stat (%)", "0"},
			{"Entry", "Reload Stat (%)", "0"},
			{"Entry", "Deploy Stat (%)", "0"},
			{"Drop", "Custom Talents (%)", {"True", "False"}, "False"},
			{"Drop", "Talent 1", {}, "None"},
			{"Drop", "Talent 2", {}, "None"},
			{"Drop", "Talent 3", {}, "None"},
			{"Drop", "Talent 4", {}, "None"},
			{"Drop", "Talent 5", {}, "None"},
			{"Drop", "Hide Chat", {"True", "False"}, "True"},
		}},
		{"SpawnUnique", "Spawn in a Unique Item", true, {
			{"Entry", "SteamID", "STEAM_0:0:"},
			{"Drop", "Unique Item", {}, "Vixen"},
			{"Drop", "Custom Stats", {"True", "False"}, "False"},
			{"Entry", "Damage Stat (%)", "0"},
			{"Entry", "RPM Stat (%)", "0"},
			{"Entry", "Ammo Stat (%)", "0"},
...
function MGA.RebuildCommandArguments(pnl, cmdd)
	...
	for i = 1, #cmd do
		...
		elseif (cmd[i][1] == "Drop") then
			...
			if (cmd[i][2] == "Tiered Item") then
				cmd[i][3] = table.GetKeys(moat_droptable_names_tier)
			end
			if (cmd[i][2] == "Unique Item") then
				cmd[i][3] = table.GetKeys(moat_droptable_names_unique)
			end
			if(cmd[i][2] == "Weapon Type") then
				cmd[i][3] = table.GetKeys(moat_weapontable_names)
			end
			if(cmd[i][2] == "Talent 1" or cmd[i][2] == "Talent 2" or cmd[i][2] == "Talent 3" or cmd[i][2] == "Talent 4" or cmd[i][2] == "Talent 5") then
				cmd[i][3] = table.GetKeys(moat_talenttable_names) 
			end
			if (cmd[i][2] == "Role") then 
				cmd[i][3] = {"innocent", "traitor", "detective", "jester"}
			end
			if (cmd[i][2] == "Choose Map") then
				cmd[i][3] = MGA.Maps or {}
			end
			if(cmd[i][2] == "Weapon Slot") then
				cmd[i][3] = {"primary", "secondary", "melee"}
			end
...
function mga.Main.Open()
	...
	mga.bg = ux.Create("DFrame", function(s)
		...
		draw.WebImage("", 0, 0, 2048, 2048, Color(255, 255, 255, 250))
		*/
