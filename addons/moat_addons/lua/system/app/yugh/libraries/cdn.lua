...
function cdn.Fetch(key, folder, ext, cache, write, callback)
	...
	elseif (file.Exists('bungottt_cache'..(ext=='.txt' and '/2/' or '/1/')..hashy..ext, "DATA")) then
		object = 'bungottt_cache'..(ext=='.txt' and '/2/' or '/1/')..hashy..ext
		cdn.Cache[key] = cache and cache(object) or ("data/" .. object)
